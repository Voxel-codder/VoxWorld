...
use voxworld_client::singleplayer::SingleplayerState;
use voxworld_client::{
    GlobalState,
    audio::AudioFrontend,
    cli, panic_handler,
...
#[cfg(feature = "egui-ui")]
use voxworld_client::ui::egui::EguiState;
...
fn main() {
    ...
    // Determine the client config directory either by env var or placed in Vox World's
    // userdata folder
    ...
    // Create window
    use voxworld_client::{error::Error, render::RenderError};
    ...
    let clipboard = voxworld_client::ui::ice::Clipboard::connect(window.window());
    ...
    let discord = if settings.networking.enable_discord_integration {
        voxworld_client::discord::Discord::start(&tokio_runtime)
    } else {
        voxworld_client::discord::Discord::Inactive
    };
