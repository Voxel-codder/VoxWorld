...
main-notice =
    欢迎来到 Vox World Alpha 版本！

    在你开始享受游戏之前，请注意以下几点：

    - 这是非常前期的 alpha 版本，你会遇到不少bug、未完成的玩法、未完善的游戏机制以及缺失的功能。

    - 如果你有建设性的意见反馈或是bug汇报，可以通过 Reddit、GitLab 或者我们的 Discord 服务器联系我们。

    - Vox World是开源的。你可以基于GPL3协议免费游玩、修改和重新发布游戏。

    - Vox World 是一个非营利的社区项目， 参与该项目的都是志愿者。
...
main-login-internal_error = 客户端内部错误。提示：玩家角色可能已被删除。
main-login-failed_auth_server_url_invalid = 无法连接到身份验证服务器。
main-login-insecure_auth_scheme = 不支持 HTTP 身份验证方案，因其存在安全隐患！出于开发目的，HTTP 仅允许用于 localhost 或调试版本。
main-login-server_full = 服务器已满。
main-login-untrusted_auth_server = 认证服务器不可信。
main-login-timeout = 超时：服务器未及时响应。提示：服务器当前可能过载或存在网络问题。
main-login-server_shut_down = 服务器已关闭。
main-login-network_error = 网络错误。
main-login-network_wrong_version = 服务器与客户端版本不匹配，请检查游戏更新。
...
main-login-username_bad_characters = 用户名包含无效字符！（仅允许字母数字、‘_’和‘-’）。
...
main-servers-stream_error = 客户端连接/压缩/解析错误：{ $raw_error }
...
loading-tips =
    ...
    .a14 = Vox World 处于Pre-Alpha阶段. 我们每天都在努力改善它!
