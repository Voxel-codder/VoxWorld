...
const VOXWORLD_USERDATA_ENV: &str = "VOXWORLD_USERDATA";
...
/// Determines common user data directory used by Vox World frontends.
///
/// The first specified in this list is used.
///   1. The VOXWORLD_USERDATA runtime environment variable
///   2. The VOXWORLD_USERDATA_STRATEGY compile time environment variable
///   3. The CARGO_WORKSPACE_DIR/userdata compile time environment variable
///      defined in .cargo/config.toml
///
/// ### `VOXWORLD_USERDATA_STRATEGY` environment variable
///
...
pub fn userdata_dir() -> PathBuf {
    // 1. The VOXWORLD_USERDATA runtime environment variable
    std::env::var_os(VOXWORLD_USERDATA_ENV)
        .map(PathBuf::from)
        // 2. The VOXWORLD_USERDATA_STRATEGY compile time environment variable
        .or_else(|| match option_env!("VOXWORLD_USERDATA_STRATEGY") {
            // "system" => system specific project data directory
            Some(s) if s.eq_ignore_ascii_case("system") => Some(directories_next::ProjectDirs::from("io", "voxworld", "voxworld")
            ...
            Some(s) => {
                warn!(
                    "Compiled with an invalid VOXWORLD_USERDATA_STRATEGY: \"{}\". \
        ...
        .unwrap_or_else(|| {
            ...
            if path.exists() && exe_path.starts_with(&path) {
                ...
                warn!(
                    ...
                    and was not compiled with VOXWORLD_USERDATA_STRATEGY set to \"system\" or \"executable\". \
